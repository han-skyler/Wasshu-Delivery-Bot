...
def LineError(line):
    ...
    print("ERROR : ", line)
    if (df['line_error'].astype(int) == 7).any():
        line_before = LineChange(line_before)
        line = line_before
    elif(df['line_error'].astype(int) == 43).any():
        line_before = LineChange(line_before)
        line = line_before
        ...
        line = line_before
    if (df['line_error'].astype(int) > 7).any() and (df['rotate'].astype(float).max() <= 0.2):
        df['rotate'] = df['rotate'] + 0.003
        df.to_csv(csv_file_path, index = False)
    df = pd.read_csv(csv_file_path)    
    print(df.iloc[0,df.columns.get_loc('rotate')].astype(float))
